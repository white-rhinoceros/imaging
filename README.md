...
- `tests/Fixtures/snapshots/<handler>/*.png` — эталонные файлы, с которыми сравниваются текущие результаты тестов.
...
При обычном `composer test` запускаются тесты, которые тоже генерируют изображения, они временно хранятся в кеше и пользователю не доступны. Далее тест считывает результат из кеша, сравнивает его по MD5 с соответствующим файлом в `snapshots/` и удаляет промежуточные данные. Если снепшота нет — тест помечается `skipped`. Поэтому после любой правки логики обязательно обновляйте эталоны.
