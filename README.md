...
#### Конструктор операций (pipeline)
Для более наглядного описания цепочки преобразований используйте `ImageManager::pipeline()`. Методы возвращают сам пайплайн, поэтому их можно вызывать цепочкой:

```php
use Whiterhino\Imaging\ImageManager;
use Whiterhino\Imaging\Types\{XPositionType, YPositionType};

$pipeline = App::make(ImageManager::class, ['public'])
...
[$cached, $url] = $pipeline->runWithUrl();
```

Доступны методы `resize()`, `crop()`, `rotate()`, `watermark()` и `call()` для произвольных операций. Для перезаписи кеша используйте `->force(true)` перед вызовом `run()` или `runWithUrl()`.
...
Визуальные тесты проверяют, что генерация изображений в GD и Imagick даёт ожидаемый результат. Они используют два набора файлов (оба набора генерируются из `tests/Fixtures/source` файлов):
- `tests/Fixtures/process/<handler>/input` и `.../output` — файлы сгенерированные для прямого просмотра и сравнения, далее в тестах не используются - при желании можно удалить.
- `tests/Fixtures/snapshots/<handler>/*.png` — эталонные файлы, с которыми сравниваются текущие результаты тестов (pixel-by-pixel).
...
При обычном `composer test` запускаются тесты, которые тоже генерируют изображения, они временно хранятся в кеше и 
пользователю не доступны. Далее тест считывает результат из кеша, сравнивает его с соответствующим файлом в `snapshots/`
и удаляет промежуточные данные. Если снепшота нет — тест помечается `skipped`. Поэтому после любой правки логики 
обязательно обновляйте эталоны.
